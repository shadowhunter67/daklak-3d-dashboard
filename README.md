...
Dự án đang chuyển dần từ "dashboard bản đồ 3D" sang "nền tảng điều hành dự án trọng điểm cấp tỉnh, dùng bản đồ làm lớp ngữ cảnh" — xem [ADR 0001](docs/adr/0001-project-centric-domain.md) và [domain model](docs/domain-model.md). Tổng quan điều hành hiện tại dùng **dữ liệu minh họa deterministic** cho 9 dự án mẫu (`src/entities/project/mockPortfolio.ts`), không phải số liệu vận hành thật — xem mục "Giới hạn và roadmap" bên dưới.
...
**Nền tảng điều hành dự án trọng điểm** (`src/entities/project/`, `src/features/executive-overview/`) đang ở **Phase 2A**: domain model + validation + KPI + data-quality (Phase 1/1.5) và Executive Overview — landing mặc định mới, đọc dữ liệu qua `ProjectPortfolioSource`/`BundledProjectPortfolioSource`, hiển thị KPI danh mục, dự án cần chú ý, cảnh báo, phân bố trạng thái và sức khỏe dữ liệu — đều dùng **9 dự án dữ liệu minh họa deterministic**, không phải số liệu vận hành thật, không dùng cho quyết định quản lý thực tế. Chưa triển khai: Project Detail đầy đủ (chỉ có panel tóm tắt), global search toàn danh mục, project layer thật trên bản đồ (nút "Xem trên bản đồ" chỉ pan camera tới toạ độ, chưa có pin/lớp riêng), API thật (repository/DTO/contract test — xem "API contract gate" trong [docs/domain-model.md](docs/domain-model.md)), permission/audit thật, và authentication. Xem [ADR 0001](docs/adr/0001-project-centric-domain.md) cho lộ trình đầy đủ.
