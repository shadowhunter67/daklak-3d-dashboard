/**
 * Deterministic checksum helpers — di chuyển từ `scripts/import-data/checksum.ts` (Phase 4, ADR
 * 0007) sang `src/` ở Phase 6 để `projectPublicBundle.ts` (public projection engine, cũng nằm dưới
 * `src/entities/project/`) dùng được mà KHÔNG tạo phụ thuộc ngược `src/ -> scripts/` (kiến trúc hiện
 * tại chỉ cho phép `scripts/ -> src/`, chưa từng có chiều ngược lại). `scripts/import-data/checksum.ts`
 * giờ chỉ re-export nguyên vẹn từ đây — không đổi named export nào, không phá call site cũ
 * (`pipeline.ts`, `fileReading.ts`, `atomicOutput.ts`, `administrativeCodes.ts`).
 *
 * Hai loại checksum tách biệt: `inputPackageChecksum` (bytes gốc, đổi khi BẤT KỲ byte input nào đổi,
 * kể cả whitespace vô hại) và `normalizedContentChecksum` (chỉ nội dung nghiệp vụ đã chuẩn hoá,
 * KHÔNG bao gồm `generatedAt` — hai lần chạy trên cùng input ở hai thời điểm khác nhau phải cho cùng
