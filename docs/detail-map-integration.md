...
| Key projects (`Dự án trọng điểm (tham khảo)`)                | **Real projects, approximate geometry** — `keyProjects.ts` is a bundled FeatureCollection of ~23 prepared / under-construction infrastructure projects (expressways, high-speed rail, reservoirs, ports, industrial parks, airports…), each with its own `sourceUrl` citation. Point/corridor geometry is hand-placed from public route maps, NOT surveyed — every feature says so and the click popup repeats the caveat. Off by default; `projects` URL param; colour = status. Separate concern from the illustrative planning overlays and from the fictional demo portfolio. |
