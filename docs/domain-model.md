...
src/entities/project/
  ...
  illustrativeProjectPortfolio.ts               9 dự án minh hoạ (giao thông ×2, năng lượng, thuỷ lợi, y tế ×2,
...
`illustrativeProjectPortfolio.ts` — 9 dự án **DỮ LIỆU MINH HỌA**, deterministic (không phụ thuộc
...
mới thêm) trỏ vào một trong ba id đó. `illustrativeProjectPortfolio.test.ts` xác nhận mọi id resolve được,
...
history nhiều điểm (prj-001). `illustrativeProjectPortfolio.test.ts` có test riêng cho từng scenario.
