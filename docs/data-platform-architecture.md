...
Both layers only ever see a dataset that's already _registered_ somewhere. Neither one, by itself,
stops a developer from dropping a brand-new JSON file under `src/assets/data/` or `public/` and
importing it directly — that's what the public data file registry (next section) closes.

## Public data file registry — a different layer than the catalog

`config/public-data-files.json` is **not** the dataset catalog. The catalog
(`DatasetDescriptor` in `catalog/datasets.ts`) describes a _logical dataset_: its provenance,
classification, freshness, quality. The registry describes _physical payloads_ allowed to ship in
the public build: one entry per exact repo-relative file path, which dataset id(s) it backs, and
how it's delivered (`bundled-static` = imported into a JS chunk from `src/`; `public-static-asset` =
copied verbatim from `public/` by Vite with no import at all). A dataset can exist with no
registered file (a document-only citation); a registered file always names the dataset(s) it backs
so its classification/`requiresAuthentication`/delivery can be cross-checked against the real
catalog. See [docs/dataset-onboarding.md](dataset-onboarding.md) (step 6) for how to add an entry, and
[docs/security-architecture.md](security-architecture.md#public-data-leakage-boundary) for what the
registry does and does not protect against.

`config/public-data-files.json` is deliberately **not** a directory-prefix allowlist (an earlier
version of this scanner used `ALLOWED_JSON_PREFIXES = ['assets/data/', 'assets/maps/daklak/']`,
which meant any new file dropped under either directory and imported would pass unnoticed). Every
entry is an exact path — no wildcards, no prefixes.

`reports/public-dataset-manifest.json` is generated by `npm run generate:public-manifest`
(`scripts/generate_public_manifest.mjs`) — a deterministic command, not a `npm test` side effect.
It uses Vite's own `ssrLoadModule` (Vite is already a dependency) to load
`publicManifest.ts`'s `buildPublicBundleManifest()` from a plain `.mjs` script without adding
ts-node/tsx, then merges in the registry's file list as `registeredPublicDataFiles` and fails loudly
if the registry itself is inconsistent with the catalog (dangling dataset id, non-public
classification, checksum mismatch, etc.) before ever writing the file.
